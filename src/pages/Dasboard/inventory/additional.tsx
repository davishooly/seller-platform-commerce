...
const Additional: React.FC = ({ form, onNext, callback, setSize }: any) => {
    ...
    return (
        <Form onSubmit={handleSubmit}>
            <Form.Item validateStatus={barcodeError ? 'error' : ''} help={barcodeError || ''}>
                ...
                })(<InlineInput label="Barcode" tip="Max. 250 characters" placeholder="Your barcode number" />)}
            </Form.Item>

            <Form.Item validateStatus={gtinError ? 'error' : ''} help={gtinError || ''}>
            ...
            <Form.Item validateStatus={variantError ? 'error' : ''} help={variantError || ''}>
                {getFieldDecorator(
                    'Variation types',
                    {},
                )(
